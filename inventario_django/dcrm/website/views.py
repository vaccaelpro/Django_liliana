...
def export_users_pdf(request):
    ...
    # Título
    title = Paragraph("Reporte de Usuarios Registrados", styles['Title'])
    elements.append(title)
    elements.append(Spacer(1, 20))

    # Datos
    data = [['Documento', 'Nombres', 'Apellidos', 'Correo', 'Rol', 'Fecha Registro']]
    users = Usuario.objects.all().order_by('date_joined')
    for u in users:
        data.append([
            u.documento, u.first_name, u.last_name, u.email, 
    ...
    # Tabla
    table = Table(data, colWidths=[80, 100, 120, 160, 90, 110])
    ...
    registrar_log(request.user, 'REPORTE', 'Exportó listado de usuarios a PDF', request)
    return response
...
def export_logs_pdf(request):
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = 'attachment; filename="logs_auditoria.pdf"'

    doc = SimpleDocTemplate(response, pagesize=landscape(letter))
    elements = []
    styles = getSampleStyleSheet()
    
    title = Paragraph("Reporte de Logs de Auditoría", styles['Title'])
    elements.append(title)
    elements.append(Spacer(1, 20))

    data = [['Fecha', 'Usuario', 'Acción', 'Detalle', 'IP']]
    logs = LogAuditoria.objects.all().order_by('-fecha')
    for log in logs:
        usuario_str = log.usuario.first_name if log.usuario else "—"
        data.append([
    ...
    table = Table(data, colWidths=[100, 100, 100, 300, 100])
    table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor("#2E8B57")),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
        ('ALIGN', (0, 0), (-1, 0), 'CENTER'),
    ...
    registrar_log(request.user, 'REPORTE', 'Exportó logs de auditoría a PDF', request)
    return response
